...
import { LanguageProvider, useLanguage } from "@/components/menu/language-provider"
      ...
      <main className="min-h-dvh bg-background px-4 py-8 sm:px-6 sm:py-12">
        <div className="mx-auto flex min-h-[calc(100dvh-4rem)] max-w-5xl flex-col justify-center">
          ...
          <header className="mb-8 text-center sm:mb-12">
            <p className="text-xs font-medium uppercase tracking-[0.28em] text-primary">{t("restaurants")}</p>
            <h1 className="mt-3 font-serif text-4xl text-foreground sm:text-6xl">{t("chooseRestaurant")}</h1>
          ...
          {restaurants.map((restaurant) => (
            ...
            >
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-8">
                <p className="text-xs uppercase tracking-[0.2em] text-white/70">{t("menu")}</p>
                ...
                <span className="mt-5 inline-flex min-h-11 items-center rounded-full bg-white px-5 text-sm font-medium text-black">{t("viewMenu")}</span>
              </div>
            </Link>
          ))}
        </div>
